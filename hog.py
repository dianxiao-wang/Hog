...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while counter < num_rolls:
        result.append(dice()) # store each result in a list
        counter += 1
    if 1 in result:
        return 1
    else:
...
def picky_piggy(score):
    ...
    if score == 0:
        ...
        decimal = float (1/7)
        power = (score % 6) if score % 6 != 0 else 6
        return (int(decimal * (10**power)) % 10) # if score = 3, 0.14285 -> 142.85 -> 142 -> 2
    # END PROBLEM 2
    ...
    def say(score0, score1):
        if who == 0:
            ...
            if gain > running_high:
                ...
                return announce_highest(who, score0, gain) #score0 becomes the new last_score in the next round
            else:
                return announce_highest(who, score0, running_high) #running_high does not update because current gain is not greater than running_high
        if who == 1:
